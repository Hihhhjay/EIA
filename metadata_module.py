...
def _get_hidden_excel_rows(xlsx_path: str, sheet_name: str) -> Set[int]:
    """
    Return 1-based Excel row numbers that are hidden in the specified sheet.
    """
...
def load_metadata(
    xlsx_path: str,
    sheet_name: str = "preliminary trial",
    ...
    skip_hidden_rows : whether to ignore Excel-hidden rows (default = True)
    """
    df = pd.read_excel(xlsx_path, sheet_name=sheet_name)
    df.columns = [str(c).strip() for c in df.columns]

    if skip_hidden_rows:
        hidden_rows = _get_hidden_excel_rows(xlsx_path, sheet_name)
        if hidden_rows:
            # pandas row index 0 corresponds to Excel row 2 (row 1 is header).
            hidden_df_indices = {r - 2 for r in hidden_rows if r >= 2}
            keep_index = [i for i in df.index if i not in hidden_df_indices]
            dropped = len(df.index) - len(keep_index)
            if dropped > 0:
                df = df.loc[keep_index].reset_index(drop=True)
                print(
                    f"[MetadataModule] Dropped {dropped} hidden Excel rows from '{sheet_name}'."
                )
